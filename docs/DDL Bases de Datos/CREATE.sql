...
CREATE TABLE COLECCIONISTA(
     ...
     CONSTRAINT COLECCIONISTA_REPRESENTANTE_FK FOREIGN KEY( REPRESENTANTE_documento_identidad ) REFERENCES REPRESENTANTE( documento_identidad ) ,
...
CREATE INDEX index_LUGAR_LUGAR_FK ON lugar( LUGAR_id );

CREATE INDEX index_CLUB_LUGAR_FK ON club( LUGAR_id );

CREATE INDEX index_CLU_INT_CLUB_FK ON clu_int( CLUB_id );
CREATE INDEX index_CLU_INT_INTERES_FK ON clu_int( INTERES_id );

CREATE INDEX index_COLECCIONISTA_COLECCIONISTA_FK ON coleccionista( COLECCIONISTA_documento_identidad );
CREATE INDEX index_COLECCIONISTA_LUGAR_FK ON coleccionista( LUGAR_id_direccion );
CREATE INDEX index_COLECCIONISTA_LUGAR_FKv2 ON coleccionista( LUGAR_id_nacionalidad );
CREATE INDEX index_COLECCIONISTA_REPRESENTANTE_FK ON coleccionista( REPRESENTANTE_documento_identidad );

CREATE INDEX index_CONTACTO_CLUB_FK ON contacto( CLUB_id );

CREATE INDEX index_DUENO_HISTORICO_COLECCIONABLE_FK ON dueno_historico( COLECCIONABLE_id );
CREATE INDEX index_DUENO_HISTORICO_COLECCIONISTA_FK ON dueno_historico( COLECCIONISTA_documento_identidad );
CREATE INDEX index_DUENO_HISTORICO_COMIC_FK ON dueno_historico( COMIC_id );

CREATE INDEX index_LOCAL_COLECCIONISTA_FK ON local( COLECCIONISTA_documento_identidad ) ;
CREATE INDEX index_LOCAL_LUGAR_FK ON local( LUGAR_id );

CREATE INDEX index_MEMBRESIA_CLUB_FK ON membresia( CLUB_id );
CREATE INDEX index_MEMBRESIA_CLUB_FK_LIDER ON membresia( CLUB_id_lider );
CREATE INDEX index_MEMBRESIA_COLECCIONISTA_FK ON membresia( COLECCIONISTA_documento_identidad );

CREATE INDEX index_SUBASTA_LOCAL_FK ON subasta( LOCAL_id );

CREATE INDEX index_PARTICIPANTE_MEMBRESIA_FK ON participante( MEMBRESIA_COLECCIONISTA_documento_identidad, MEMBRESIA_CLUB_id, MEMBRESIA_fecha_ingreso );
CREATE INDEX index_PARTICIPANTE_SUBASTA_FK ON participante( SUBASTA_id );

CREATE INDEX index_ORG_INV_CLUB_FK ON org_inv( CLUB_id_org );
CREATE INDEX index_ORG_INV_CLUB_FKv2 ON org_inv( CLUB_id_inv );
CREATE INDEX index_ORG_INV_SUBASTA_FK ON org_inv( SUBASTA_id );

CREATE INDEX index_ORG_SUB_ORGANIZACION_CARIDAD_FK ON org_sub( ORGANIZACION_CARIDAD_id );
CREATE INDEX index_ORG_SUB_SUBASTA_FK ON org_sub( SUBASTA_id );

CREATE INDEX index_LISTADO_DUENO_HISTORICO_FK ON listado( DUENO_HISTORICO_COLECCIONISTA_documento_identidad, DUENO_HISTORICO_fecha_registro, DUENO_HISTORICO_id );
CREATE INDEX index_LISTADO_PARTICIPANTE_FK ON listado( PARTICIPANTE_subasta_id, PARTICIPANTE_id_inscripcion );
CREATE INDEX index_LISTADO_SUBASTA_FK ON listado( SUBASTA_id );
